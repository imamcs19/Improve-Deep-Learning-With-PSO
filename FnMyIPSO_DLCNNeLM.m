function ResultMADgBest=FnMyIPSO_DLCNNeLM(typeFitur)
% close all
% clear all
% clc
...
for t=0:IterMaxPSO
    ...
    if(t==0)    
        ...
        if(mod(t,5)==0) % lakukan random injection
            ...
            % hitung nilai fitness X
            [FitnessAllX,IndexSortingDesc]=FnGetFitnessIPSOSVM(X);
            
            % update Pbest dan Gbest
            [FitnessAll_Update_Pbest,Fitness_Update_Gbest,Update_Pbest,Update_Gbest]=...
                FnUpdatePbestGbestIPSOSVM(FitnessAllPbest,FitnessAllX,FitnessGbest,X,Pbest,Gbest);
            FitnessAllPbest=FitnessAll_Update_Pbest;
            FitnessGbest=Fitness_Update_Gbest;
            Pbest=Update_Pbest;
            Gbest=Update_Gbest;
